...
//	fp=fopen("results.csv","w");
	...
	printf("%d",n_requests);
	prefix_len = sprintf(prefix, "/tmp//");
	strcpy(url,prefix);
for (i = 0; i < n_requests; i++) {
sprintf(url + prefix_len, "%d", __sync_add_and_fetch(&total_n_launched, 1));
	printf("%s\n",url);
	ret=acrd_read(ri->bh,url,&readbuf,read_msg_size,0,NULL);
		printf("%d",ret);
	...
	/*acrd_list(ri->bh,url,NULL,NULL);
		ret= acrd_list(ri->bh,url,ACRD_FLAG_SYNC,NULL);
			if (ret != ACRD_SUCCESS) {
			eprintf("err, %d\n", ret);
			exit(1);
			}*/
