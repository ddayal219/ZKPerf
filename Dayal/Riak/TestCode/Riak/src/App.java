...
public class App {
    ...
    public static void main(String[] args) throws RiakException, IOException {
        ...
        csv.append("Test Name,Backend,Number of KV,HTTP Interface Time(s),HTTP Interface throughput, PB Time(s), PB throughput(s)");
        csv.append("\n");
        for (i = 1; i <= LoopDelimiter; i = (long) (Math.pow(exponentBase, j / 2))) {
            ...
            for (k = 0; k < i; k++) {
                ...
                myBucket.store("TestKey", myData).w(1).execute();
            }
            ...
            for (k = 0; k < i; k++) {
                pbBucket.store("TestKey1", myData).execute();
            }
            diffInSeconds1 = (double) (System.currentTimeMillis() - start1) / 1000;
            csv.append("Riak Performance Test-Write,memory," + i + "," + diffInSeconds + "," + Math.ceil((double) i / (diffInSeconds)) + "," + diffInSeconds1 + "," + Math.ceil((double) i / (diffInSeconds1)));
