...
public class aSyncPrimitive2 implements Watcher {

    static ZooKeeper zk = null;
    static Integer mutex;
    String root;
   static Queue q1;
    aSyncPrimitive2(String address) {
        if (zk == null) {
            try {
     ...
     */
    static public class Queue extends aSyncPrimitive2 {
...
//System.out.println("difference"+diff);
    ...
    public static void queueTest(String args[]) throws Exception {
        final Queue q = new Queue(args[0], "/app1");
            aSyncPrimitive2.q1=q;
	double cdiffInSeconds1=0.0;
        //System.out.println("Input: " + args[0]);
        long k, i = 0;
        Integer max = new Integer(10000);
        long start = 0, end = 0, diff = 0,smax=0,gcdiff=0,ldiff=0;
        double diffInSeconds = 0.0, j = 0.0,gcdiffInSeconds=0.0,ldiffInSeconds=0.0;
...
z++;
 ...
 while (gCounter.get() != i) {
	...
	gcdiff=System.currentTimeMillis()-Starting;
	gcdiffInSeconds=((double)gcdiff)/1000;
...
System.out.println(cdiffInSeconds1);
...
ldiff=System.currentTimeMillis()-start;
ldiffInSeconds=((double)ldiff)/1000;
            } else {
...
//System.out.println(smax);
            ...
            diffInSeconds = ((double) smax) / 1000;
          ...
          //  System.out.println(i + "\t\t    " + (diffInSeconds) + "\t\t" + Math.ceil((double) i / (diffInSeconds))+"\t\t"+Math.ceil((double) i/(diffInSeconds1)));
            ...
            System.out.println(i + "\t\t" + diffInSeconds + "\t\t"+ cdiffInSeconds1 + "\t\t" + gcdiffInSeconds + "\t\t" + ldiffInSeconds + "\t\t"+ Math.ceil((double) i / (diffInSeconds)));
            j++;
	z=0;smax=0;
        }
        csv.close();
